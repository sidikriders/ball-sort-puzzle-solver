const ModalEditTube = (props) => {
  const { visible, onClose, size, tubes, tubeIndex, saveTube } = props
      ...
      <div className="modal-content">
        ...
        <div className="tube-editor">
          <div className="balls-selections">
            {
              colors.map((color, idx) => {
                return (
                <button className="button" key={idx} onClick={() => addBall(color)}>
                  <Ball color={color} />
                </button>
                )
              })
          ...
          <div className="tube-container">
            <Tube balls={balls} editor onBallClick={(ballIndex) => removeBall(ballIndex)}/>
          </div>
        </div>
        <div className="edit-tube-action">
          <button className="button is-danger" onClick={() => onClose()}>Cancel</button>
          <button className="button is-primary" onClick={() => saveTube(balls)}>{tubeIndex < 0 ? 'Add New Tube' : 'Update Tube'}</button>
        </div>
      </div>
    </div>
  )
